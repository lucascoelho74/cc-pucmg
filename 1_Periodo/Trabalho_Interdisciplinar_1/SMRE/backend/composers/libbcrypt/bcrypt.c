 ...
 */
/* Patch local (SixSeven): no Windows nao existe /dev/urandom; usamos
 * rand_s() (CSPRNG do CRT). _CRT_RAND_S precisa vir antes de stdlib.h. */
#ifdef _WIN32
#define _CRT_RAND_S
#include <stdlib.h>
#endif
...
#ifdef _WIN32
	/* Patch local (SixSeven): bytes aleatorios via rand_s (CSPRNG). */
	{
		unsigned int r;
		size_t i;
		for (i = 0; i < RANDBYTES; i += sizeof(r)) {
			if (rand_s(&r) != 0)
				return 1;
			memcpy(input + i, &r, sizeof(r));
		}
	}
#else
	int fd;
